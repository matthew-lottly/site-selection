...
Colors reuse this project's own validated palette (lib.py's SEQUENTIAL_BLUE,
...
from lib import PROCESSED, ROOT, STATUS_RAMP, ramp_color
...
# ---------------------------------------------------------------------------
def chart_pipeline_funnel() -> None:
    tracts = load("tracts.csv")
    houston_tracts = load("houston_tracts.csv")
    clusters = load("clusters.csv")
    parcels = load("candidate_parcels.csv")
    shortlist = load("site_shortlist.csv")
    ...
    for i, (label, n) in enumerate(stages):
        ...
        color = ramp_color(i / (len(stages) - 1), STATUS_RAMP[::-1] if False else ["#184f95", "#256abf", "#3987e5", "#6da7ec", "#9ec5f4", GOLD])
        left = i - width / 2
...
def chart_site_map() -> None:
    scorecard = load("scorecard.csv")
    sites = {r["hcad_num"]: r for r in load("sites_enriched.csv")}
    clusters = load("clusters.csv")
    rings = _load_boundary_rings()
    ...
    for row in scorecard:
        ...
        if is_winner:
            ...
            color = ramp_color((rank - 1) / max(n - 1, 1), STATUS_RAMP)
    ...
    handles = [
        ...
        plt.Line2D([0], [0], marker="o", color="w", markerfacecolor=STATUS_RAMP[0], markeredgecolor="white", markersize=10, label="Higher-scoring finalist"),
        plt.Line2D([0], [0], marker="o", color="w", markerfacecolor=STATUS_RAMP[-1], markeredgecolor="white", markersize=10, label="Lower-scoring finalist"),
    ...
    save(fig, "site_map.png")


# ---------------------------------------------------------------------------
def chart_top5_scores() -> None:
    scorecard = sorted(load("scorecard.csv"), key=lambda r: int(r["raw_score_rank"]))[:5]
    ...
    save(fig, "top5_scores.png")


# ---------------------------------------------------------------------------
def chart_cannibalization() -> None:
    scorecard = {r["hcad_num"]: r for r in load("scorecard.csv")}
    canib = {r["hcad_num"]: r for r in load("cannibalization.csv")}
    ...
    for i, (v, rk) in enumerate(zip(vals, risk)):
        badge_color = GREEN if rk == "Low" else STATUS_RAMP[-1]
...
def chart_sensitivity() -> None:
    ...
    handles = [
        ...
        plt.Rectangle((0, 0), 1, 1, facecolor=STATUS_RAMP[1], label="A different site wins"),
    ]
...
# ---------------------------------------------------------------------------
def chart_confidence_intervals() -> None:
    rows = load("city_confidence_intervals.csv")
    ...
    save(fig, "confidence_intervals.png")


if __name__ == "__main__":
    chart_pipeline_funnel()
    chart_site_map()
    chart_top5_scores()
    chart_cannibalization()
    chart_sensitivity()
