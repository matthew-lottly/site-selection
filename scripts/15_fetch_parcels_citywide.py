...
def fetch_near(key: str, lat: float, lon: float, radius_m: int = 350) -> list[dict]:
...
def main() -> None:
    ...
    for i, x in enumerate(intersections):
        ...
        rows = fetch_near(key, float(x["lat"]), float(x["lon"]))
        new = 0
        for r in rows:
    ...
    for cluster_id, rows in by_cluster.items():
        ...
        shortlist.extend(rows[:MAX_PER_CLUSTER])
    ...
    print(f"Wrote {len(shortlist)} shortlisted finalists (up to {MAX_PER_CLUSTER}/cluster) -> {shortlist_path}", file=sys.stderr)
    for r in shortlist:
