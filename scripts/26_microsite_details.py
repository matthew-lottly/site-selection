...
def query_microsite(lat: float, lon: float) -> dict:
    ...
    for attempt in range(4):
        resp = requests.post(OVERPASS, data={"data": q}, timeout=60, headers={"User-Agent": "curl/8.7.1", "Accept": "*/*"})
...
def parcel_bounding_ft(hcad_num: str) -> tuple[float, float] | None:
    ...
    for cache_file in RAW.glob("hcad_*.json"):
        ...
        for feat in data.get("features", []):
            ...
            width_mi = haversine_miles(min(lats), min(lons), min(lats), max(lons))
            height_mi = haversine_miles(min(lats), min(lons), max(lats), min(lons))
...
def main() -> None:
    ...
    for s in sites:
        ...
        for el in data["elements"]:
            t = el.get("tags", {})
            kind = POI_TAGS.get(t.get("amenity")) or ("Laundromat" if t.get("shop") == "laundry" else None)
            if not kind:
                continue
            if el["type"] == "node":
