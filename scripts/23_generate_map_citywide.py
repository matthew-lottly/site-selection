...
<script>
(function() {{
  const mapRef = {map_name};
  const femaLayerRef = {layer_name};
  ...
  async function loadFemaFromApi() {{
  ...
  mapRef.on('overlayadd', function(ev) {{
    if (ev.layer === femaLayerRef) loadFemaFromApi();
  }});

  if (mapRef.hasLayer(femaLayerRef)) loadFemaFromApi();
}})();
</script>
"""
