...
class FindIntersectionsCitywideStage(PipelineStage):
    id = "14"
    name = "find_intersections_citywide"
    description = "Auto-discover named arterial roads per cluster (Overpass) and find real intersections"
    outputs = (PROCESSED / "intersections.csv",)
    ...
    def fetch_roads(self, cluster_id: str, min_lat, min_lon, max_lat, max_lon) -> list[dict]:
        ...
        for attempt in range(5):
            resp = requests.post(self.overpass.BASE_URL, data={"data": q}, timeout=90, headers={"User-Agent": "curl/8.7.1", "Accept": "*/*"})
    ...
    def find_intersections(self, elements: list[dict]) -> list[dict]:
        ...
        for el in elements:
            ...
            if not name or not geom:
                continue
            for pt in geom:
                key = (round(pt["lat"] / self.GRID_DEG), round(pt["lon"] / self.GRID_DEG))
        ...
        for f in sorted(found, key=lambda x: (x["road_a"], x["road_b"])):
            if any(
                f["road_a"] == d["road_a"] and f["road_b"] == d["road_b"] and Geometry.haversine_miles(f["lat"], f["lon"], d["lat"], d["lon"]) < self.DEDUPE_MI
                for d in deduped
            ):
                continue
            deduped.append(f)

        if len(deduped) > self.MAX_INTERSECTIONS_PER_CLUSTER:
            deduped.sort(key=lambda x: (x["lat"], x["lon"]))
            stride = len(deduped) / self.MAX_INTERSECTIONS_PER_CLUSTER
            deduped = [deduped[int(i * stride)] for i in range(self.MAX_INTERSECTIONS_PER_CLUSTER)]
    ...
    def run(self) -> None:
        ...
        for c in clusters:
            ...
            xs = self.find_intersections(elements)
...
if __name__ == "__main__":
    FindIntersectionsCitywideStage().run()
