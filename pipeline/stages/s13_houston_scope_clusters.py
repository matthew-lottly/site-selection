...
class HoustonScopeClustersStage(PipelineStage):
    id = "13"
    name = "houston_scope_clusters"
    description = "Scope analysis to real Houston city limits and identify citywide opportunity clusters"
    outputs = (
        PROCESSED / "houston_boundary.geojson",
        PROCESSED / "houston_tracts.csv",
    ...
    def run(self) -> None:
        ...
        bboxes = [Geometry.ring_bbox(r) for r in rings]
        ...
        simplified_rings = [Geometry.simplify_ring(r, self.SIMPLIFY_TOLERANCE_DEG) for r in rings]
        ...
        for t in tracts:
            ...
            if Geometry.point_in_rings_fast(lat, lon, rings, bboxes):
                in_city.append(t)
        ...
        candidates = sorted(
            [r for r in in_city if density(r) >= self.MIN_TRACT_DENSITY_PER_SQMI],
            key=lambda r: -float(r["gap_score"]),
        )[:250]

        grid: dict[tuple, list] = defaultdict(list)
        for r in candidates:
            key = (round(float(r["lat"]) / self.GRID_CELL_DEG), round(float(r["lon"]) / self.GRID_CELL_DEG))
            grid[key].append(r)
        ...
        for c in cell_clusters:
            if any(Geometry.haversine_miles(c["lat"], c["lon"], p["lat"], p["lon"]) < self.MIN_CLUSTER_SEPARATION_MI for p in picked):
                continue
            picked.append(c)
            if len(picked) >= self.N_CLUSTERS:
                break
        ...
        for i, c in enumerate(picked, start=1):
            ...
            pad = 0.045
            neighborhood = self.reverse_geocode_neighborhood(c["lat"], c["lon"])
            rows.append(
                {
                    "cluster_id": f"C{i}",
                    "score": round(c["score"]),
