...
class CannibalizationAnalysisStage(PipelineStage):
    id = "24"
    name = "cannibalization_analysis"
    description = "Cannibalization-risk analysis against Family Dollar's own existing store network"
    outputs = (PROCESSED / "cannibalization.csv",)
    ...
    @classmethod
    def risk_category(cls, nearest_fd_mi: float, overlap_pct: float) -> str:
        if nearest_fd_mi < cls.HARD_BUFFER_HIGH_MI:
            return "High (hard buffer: existing FD < 1.2 mi)"
        if nearest_fd_mi > cls.HARD_BUFFER_CLEAR_MI and overlap_pct < cls.OVERLAP_LOW_PCT:
            return "Low"
        if overlap_pct > cls.OVERLAP_HIGH_PCT:
            return "High (trade-area overlap)"
        if overlap_pct >= cls.OVERLAP_LOW_PCT:
            return "Moderate"
        return "Low"
    ...
    def run(self) -> None:
        ...
        for s in sites:
            ...
            nearby_bgs = [b for b in bgs if Geometry.haversine_miles(lat, lon, float(b["lat"]), float(b["lon"])) <= 10.0]
            ...
            for bg, dur in zip(nearby_bgs, durations[1:]):
                ...
                pop_5min += pop
                d_to_fd = Geometry.haversine_miles(float(bg["lat"]), float(bg["lon"]), float(nearest_fd["lat"]), float(nearest_fd["lon"]))
                if d_to_fd <= self.OVERLAP_RADIUS_MI:
                    overlap_pop += pop

            overlap_pct = (overlap_pop / pop_5min * 100) if pop_5min else 0.0
            net_new_pop = pop_5min - overlap_pop
            risk = self.risk_category(nearest_fd_mi, overlap_pct)
            ...
            rows.append(
                {
                    ...
                    "hard_buffer_flag": "High risk (<1.2mi)" if nearest_fd_mi < self.HARD_BUFFER_HIGH_MI else ("Clear (>2.5mi)" if nearest_fd_mi > self.HARD_BUFFER_CLEAR_MI else "Soft buffer, check overlap"),
...
if __name__ == "__main__":
    CannibalizationAnalysisStage().run()
