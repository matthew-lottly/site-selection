...
from ..clients import FemaClient
from ..core import PROCESSED, RAW, PipelineStage
from ..geometry import Geometry


class EnrichSitesCitywideStage(PipelineStage):
    id = "18"
    name = "enrich_sites_citywide"
    description = "FEMA flood zone, verified nearest AADT, and competitor distances for all citywide finalists"
    outputs = (PROCESSED / "sites_enriched.csv",)
    ...
    def run(self) -> None:
        ...
        for i, s in enumerate(sites, start=1):
            ...
            neighborhood = self.reverse_geocode_neighborhood(lat, lon)
            ...
            zone, sfha = self.fema_flood_zone(lat, lon)

            ranked_aadt = sorted(aadt, key=lambda a: Geometry.haversine_miles(lat, lon, float(a["lat"]), float(a["lon"])))[:8]
            nearest_aadt = None
            aadt_road_name = None
            aadt_on_freeway = False
            for candidate in ranked_aadt:
                name = self.reverse_geocode_road(float(candidate["lat"]), float(candidate["lon"]))
                if not self.is_true_freeway(name):
            ...
            if nearest_aadt is None:  # every nearby station is on a freeway; fall back to nearest, flagged
                ...
                aadt_road_name = self.reverse_geocode_road(float(nearest_aadt["lat"]), float(nearest_aadt["lon"]))
                aadt_on_freeway = True
            aadt_dist = Geometry.haversine_miles(lat, lon, float(nearest_aadt["lat"]), float(nearest_aadt["lon"]))

            nearest_dollar = min(true_competitors, key=lambda x: Geometry.haversine_miles(lat, lon, float(x["lat"]), float(x["lon"])))
            nearest_dollar_mi = Geometry.haversine_miles(lat, lon, float(nearest_dollar["lat"]), float(nearest_dollar["lon"]))

            nearest_fd = min(family_dollar, key=lambda x: Geometry.haversine_miles(lat, lon, float(x["lat"]), float(x["lon"])))
            nearest_fd_mi = Geometry.haversine_miles(lat, lon, float(nearest_fd["lat"]), float(nearest_fd["lon"]))

            nearest_grocery = min(value_grocery, key=lambda x: Geometry.haversine_miles(lat, lon, float(x["lat"]), float(x["lon"]))) if value_grocery else None
            nearest_grocery_mi = Geometry.haversine_miles(lat, lon, float(nearest_grocery["lat"]), float(nearest_grocery["lon"])) if nearest_grocery else None

            nearest_bigbox = min(big_box, key=lambda x: Geometry.haversine_miles(lat, lon, float(x["lat"]), float(x["lon"])))
            nearest_bigbox_mi = Geometry.haversine_miles(lat, lon, float(nearest_bigbox["lat"]), float(nearest_bigbox["lon"]))
    ...
    def is_true_freeway(self, name: str) -> bool:
        ...
        if "frontage" in name.lower():
            return False
        return bool(self.FREEWAY_PATTERN.search(name))

    def fema_flood_zone(self, lat: float, lon: float) -> tuple[str, str]:
        data = self.get_json_with_retry(
            self.fema.BASE_URL,
            params={
                "geometry": f"{lon},{lat}",
    ...
    def reverse_geocode(self, lat: float, lon: float, zoom: int) -> dict:
        cache_path = RAW / f"nominatim_{zoom}_{round(lat, 5)}_{round(lon, 5)}.json"
        if cache_path.exists():
            import json

            return json.loads(cache_path.read_text(encoding="utf-8"))
        d = self.get_json_with_retry(
    ...
    def reverse_geocode_road(self, lat: float, lon: float) -> str:
        d = self.reverse_geocode(lat, lon, zoom=17)
        addr = d.get("address", {})
        return addr.get("road") or d.get("display_name", "").split(",")[0] or "unknown road"

    def reverse_geocode_neighborhood(self, lat: float, lon: float) -> str:
        d = self.reverse_geocode(lat, lon, zoom=15)
        addr = d.get("address", {})
        return (
            addr.get("suburb")
