...
from ..clients import TigerWebClient
from ..core import PROCESSED, RAW, PipelineStage
from ..geometry import Geometry


class FetchHoustonTractGeometryStage(PipelineStage):
    id = "21"
    name = "fetch_houston_tract_geometry"
    description = "Real tract polygon geometry for every Census tract inside Houston city limits"
    outputs = (PROCESSED / "houston_tracts.geojson",)
    ...
    def run(self) -> None:
        ...
        data = self.tigerweb.query(
            8,
            {
                "geometry": json.dumps(self.ENVELOPE),
        ...
        for f in data["features"]:
            ...
            if not Geometry.point_in_rings_fast(lat, lon, rings, bboxes):
                continue
            simplified_rings = [Geometry.simplify_ring(r, self.SIMPLIFY_TOLERANCE_DEG) for r in f["geometry"]["rings"]]
            features.append(
                {
                    "type": "Feature",
...
if __name__ == "__main__":
    FetchHoustonTractGeometryStage().run()
