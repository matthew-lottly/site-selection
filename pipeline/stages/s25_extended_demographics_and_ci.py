...
class ExtendedDemographicsAndCiStage(PipelineStage):
    id = "25"
    name = "extended_demographics_and_ci"
    description = "Houston tract-level extended ACS demographics + city-wide 90% confidence intervals"
    outputs = (PROCESSED / "city_confidence_intervals.csv", PROCESSED / "tract_extended_demographics.csv")
    ...
    def run(self) -> None:
        ...
        city = self.census.fetch_batch(self.TABLES, ["4835000"], geo_prefix="16000US", release="acs2024_5yr")
        d = city["16000US4835000"]
        ...
        poverty_rate_moe = self.ratio_moe(pov_below_est, pov_below_moe, pov_tot_est, pov_tot_moe)
        fb_rate = fb_est / fb_tot_est
        fb_rate_moe = self.ratio_moe(fb_est, fb_moe, fb_tot_est, fb_tot_moe)
        span_rate = span_est / lang_tot_est
        span_rate_moe = self.ratio_moe(span_est, span_moe, lang_tot_est, lang_tot_moe)
...
if __name__ == "__main__":
    ExtendedDemographicsAndCiStage().run()
