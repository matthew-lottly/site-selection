...
class DriveTimesAndHuffStage(PipelineStage):
    id = "19"
    name = "drive_times_and_huff"
    description = "Real OSRM drive-time trade areas + Huff gravity market-capture model for all citywide finalists"
    outputs = (PROCESSED / "site_trade_areas.csv",)
    ...
    def run(self) -> None:
        ...
        for s in sites:
            ...
            nearby_bgs = [b for b in bgs if Geometry.haversine_miles(lat, lon, float(b["lat"]), float(b["lon"])) <= self.BLOCKGROUP_PREFILTER_MI]
            ...
            if cache_path.exists():
                ...
                for start in range(0, len(nearby_bgs), CHUNK):
                    ...
                    data = self.osrm.table(coords, {"sources": "0", "annotations": "duration"}, retries=5, timeout=60)
            ...
            nearby_competitors = [
                ...
                and Geometry.haversine_miles(lat, lon, float(c["lat"]), float(c["lon"])) <= self.COMPETITOR_SEARCH_RADIUS_MI
            ]
            ...
            for bg, dur in zip(nearby_bgs, durations[1:]):
                ...
                for c in nearby_competitors:
                    d_mi = Geometry.haversine_miles(float(bg["lat"]), float(bg["lon"]), float(c["lat"]), float(c["lon"]))
                    t_c_min = (d_mi / 25.0) * 60.0  # 25 mph average urban-arterial speed assumption
                    comp_list.append((t_c_min, float(c["typical_sqft"])))
                p = self.huff_probability(t_site_min, comp_list)
    ...
    def huff_probability(self, t_site_min: float, competitors_min_sqft: list[tuple[float, float]]) -> float:
        t = max(t_site_min, 0.5)
        site_attraction = self.CANDIDATE_SQFT / (t**self.BETA)
        comp_attraction = sum(sqft / (max(t_c, 0.5) ** self.BETA) for t_c, sqft in competitors_min_sqft)
        total = site_attraction + comp_attraction
        return site_attraction / total if total > 0 else 0.0


if __name__ == "__main__":
    DriveTimesAndHuffStage().run()
