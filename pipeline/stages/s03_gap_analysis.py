...
class GapAnalysisStage(PipelineStage):
    ...
    def run(self) -> None:
        ...
        dollar_stores = [s for s in stores if s["category"] in self.DOLLAR_FORMAT_CATEGORIES]
        ...
        for t in tracts:
            ...
            if not dollar_stores:
                ...
                nearest_dollar_mi = min(Geometry.haversine_miles(lat, lon, float(s["lat"]), float(s["lon"])) for s in dollar_stores)

            demand_index = pop * self.income_fit(mhi) * (1 + poverty_rate)
