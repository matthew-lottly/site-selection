...
class StatisticalRigorStage(PipelineStage):
    id = "29"
    name = "statistical_rigor"
    description = "Spatial autocorrelation (Moran's I), spatial block CV, and OLS fit diagnostics on gap_score"
    outputs = (
        PROCESSED / "statistical_rigor.csv",
        PROCESSED / "statistical_rigor_folds.csv",
    )
    ...
    def osrm_drive_minutes(self, site_lat: float, site_lon: float, target_lat: float, target_lon: float, cache_key: str) -> float | None:
        safe_key = "".join(ch if ch.isalnum() or ch in {"-", "_"} else "_" for ch in cache_key)
        data = self.osrm.route_cached(site_lat, site_lon, target_lat, target_lon, f"osrm_competitor_{safe_key}", timeout=25)
        if data is None:
            return None
    ...
    def permutation_pvalue(self, values: np.ndarray, weights: np.ndarray, observed: float, n_perm: int = 199) -> float:
        rng = np.random.default_rng(42)
        extreme = 0
        for _ in range(n_perm):
            perm = rng.permutation(values)
            score = self.morans_i(perm, weights)
    ...
    def run(self) -> None:
        rows = self.load_rows(PROCESSED / "houston_tracts.csv")
        ...
        beta = self.fit_ols(x, y)
        y_hat = x @ beta
        residuals = y - y_hat

        ins_rmse = self.rmse(y, y_hat)
        ins_r2 = self.r2(y, y_hat)

        coords = np.column_stack([lat_arr, lon_arr])
        w = self.build_knn_weights(coords, k=8)
        i_obs = self.morans_i(residuals, w)
        i_expected = -1.0 / (len(residuals) - 1)
        i_p = self.permutation_pvalue(residuals, w, i_obs, n_perm=199)

        block_ids = self.spatial_blocks(lat_arr, lon_arr, n_bins=4)
        ...
        for block in unique_blocks:
            ...
            beta_fold = self.fit_ols(x[train_mask], y[train_mask])
            pred = x[test_mask] @ beta_fold
            y_test = y[test_mask]

            fold_rmse = self.rmse(y_test, pred)
            fold_r2 = self.r2(y_test, pred)
        ...
        scorecard = self.load_rows(PROCESSED / "scorecard.csv")
        sites = {r["hcad_num"]: r for r in self.load_rows(PROCESSED / "sites_enriched.csv")}
        competitors = self.load_rows(PROCESSED / "competitors.csv")
        ...
        nearest_arch = min(
            arch,
            key=lambda c: Geometry.haversine_miles(wlat, wlon, float(c["lat"]), float(c["lon"])),
        )
        nearest_arch_mi = Geometry.haversine_miles(wlat, wlon, float(nearest_arch["lat"]), float(nearest_arch["lon"]))
        nearest_arch_drive_min = self.osrm_drive_minutes(
            wlat,
            wlon,
