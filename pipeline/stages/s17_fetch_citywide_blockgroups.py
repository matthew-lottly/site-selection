...
class FetchCitywideBlockgroupsStage(PipelineStage):
    id = "17"
    name = "fetch_citywide_blockgroups"
    description = "Citywide Harris County block groups inside Houston city limits + ACS 5yr demographics"
    outputs = (PROCESSED / "blockgroups.csv",)
    ...
    def run(self) -> None:
        ...
        in_city = [
            ...
            if Geometry.point_in_rings_fast(float(bg["CENTLAT"]), float(bg["CENTLON"]), rings, bboxes)
        ]
        ...
        if demo_cache.exists():
            ...
            for i in range(0, len(geoids), batch_size):
                ...
                result = self.census.fetch_batch("B01003,B19013", batch, geo_prefix="15000US")
...
if __name__ == "__main__":
    FetchCitywideBlockgroupsStage().run()
